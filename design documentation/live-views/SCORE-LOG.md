...
- **Motion proof captured (closes that residual):** a 20×-scaled freeze sequence (`captures/frontend-v2/r5_motion_t0{0,5,6,8,11}*.png`) shows the pivot ring **faint at t05** (the `ov-3` "overturned to BREACHED" beat resolving) then **snapped bold at t06** — the ringed turn and the verdict sentence land together, exactly as the keyframe math predicts. Normal-speed `r5_overturn_motion.webm` for the user. The one thing stills couldn't witness is now witnessed.
