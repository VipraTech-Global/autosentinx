...
> **⏸️ STATUS: PARKED (decided 2026-06-20, `DECISIONS.md` D-LV26).** This is the locked *target* and a de-risking proof — **not yet a build order.** The port depends on a frozen `runview.ts`, and the contract is **not** lockable while the consuming views are in flight: **none of V1, V2 or V3 is finalized** (V1 unbuilt per `D-LV23`; V2/V3 scored 96–97 but not signed-off; `OPEN-LV1`/`OPEN-LV2` open). **Un-park trigger:** V1 built → opens resolved → V2/V3 finalized → `runview.ts` frozen, *then* build Phase 1→3. Growth is additive, so waiting costs nothing here.
>
