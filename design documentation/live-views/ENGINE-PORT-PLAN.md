...
> **▶️ STATUS: UNPARKED — IMPLEMENTING (Revision R1, 2026-06-21).** The 2026-06-20 PARK (`D-LV26`) is superseded by a build directive. **R1 reconciles this plan against `origin/main`, `origin/feat/security-governance-keystones` (keystones), and the FE/contract as it stands after ~10h of live-views work.** R1 is the authoritative build order; where the EP-1…EP-16 body below conflicts with R1, **R1 wins**. (Source review: workflow `wf_c6e7569e-ce2` — 29 findings, 2 blockers, 10 major.)
>
> ## Revision R1 — reconciliation + vertical-wave build order
>
> ### Blockers absorbed
> - **`[R1-B1]` `regulation` is snake_case `{framework, control_id, control_title}`** — do **NOT** reuse `console._crosswalk` verbatim (it emits camelCase `controlId/controlTitle`); the frozen contract (`runview.ts:71`) + FE (`arena.tsx`) read snake_case incl. `control_title`. `runview.py` gets a **new** snake_case, pillar-filtered mapper. *(supersedes EP-3 JSON + EP-8)*
> - **`[R1-B2]` KEEP `fromStateJson`** — it is the live-views review/demo harness (8 `SAMPLES`, the dev `NODE_ENV`-gated switcher, 9 `public/runs/*.json` fixtures incl. `midrun`/`degraded`/`recon-demo` the engine can't reproduce on demand). Live engine is an **ADDITIONAL** source: `data='live'`/`?live=1` → `getRunView(id)`; any other `data` → fixture + `fromStateJson`. The `RunView` interface + derive helpers stay shared/unchanged. *(supersedes EP-13 step 5 "retire fromStateJson")*
>
> ### Majors absorbed
> - **`[R1-M1]` Anchor all `runner.py` cites by SYMBOL, not line** — keystones inserts ~+18 (campaign methods) to ~+35 (`_run_one`) lines (gateway/`_anchor`). Cite `_run_one`'s `for idx in range(self.s.max_turns)` loop, the early-stop guard, the post-`_oracle_verdict` return, and the recon-profile call site by name. Pure textual offset; no logic change in cited regions.
> - **`[R1-M2]` Keystones merge order (EP-R8)** — keystones gateway-wraps `target = AuditedGateway(AaravTarget(...), RoE(...), Budget(...))` in all 3 campaign methods. Build the EP waves on the **main-based integration line** and symbol-anchor the `runner.py` edits; the future keystones merge is a **mechanical, non-semantic** conflict (the gateway is API-compatible — `_run_one` uses `send_turn/end_session` only). Do not unilaterally merge keystones. *(was Wave 0 "merge first"; revised to "build main-based + document reconciliation")*
> - **`[R1-M3/M4]` The FE switch is NET-NEW behavior, not a source swap** — `getRunView` does **not** exist; the pages call `fromStateJson` **inline** in a `useEffect`; `?live=1` today re-polls a **static fixture** (no engine call anywhere in the FE). Add `getRunView(id) → /api/console/runs/${id}/runview` (returns `RunView` verbatim, **no** `withOutcomes`); rewrite the load effect in **both** `app/runs/[id]/arena/page.tsx` and `.../[playId]/forensic/page.tsx` to branch on source, preserving the role gate, `RunNav`, dev-bridge, sample switcher, and the **2500ms** poll + terminal-stop.
> - **`[R1-M5]` Live mode adds login + BFF-401 + role gates** — fixtures hit static `/runs/*.json` (no auth); live hits `/api/console/...` (BFF attaches Bearer, 401 if no `sx_jwt`). **No proxy change** (catch-all already forwards `/console/...`). Handle the 401 in the live effect → "log in to view the live run". Fixture path must keep working with the engine down.
> - **`[R1-M6]` Document the full `ReconView`** (`status/reason/profile{disclosesAi,staysInScope,refusalStyle,notes}/steps[]/links[]`) — FE renders steps/links **today** (`arena.tsx`, `forensic.tsx`); Phase-1 still ships `recon:null` (degrades OK), full ReconView in Wave 4.
> - **`[R1-minor]`** `engine.maxTurns` MUST be a non-null int even Phase-1 (the pending-ladder estimate `arena.tsx` divides by it); `getRunView` must NOT run `withOutcomes`; **do NOT copy `console.py:148`'s `error/blocked` skip** (RunView renders them — filter only `sample`); use a **model-retaining** vote mapper (keep `provider:model`); `intensity` is the 6-level enum `low|med|high|xhigh|max|ultra`; name the projector arg `run_roe` to disambiguate from keystones' `roe.py` policy module; the endpoint loads **both** `Catalog` AND `Library` (needs `phasePlan`).
>
> ### N-order dependency chain
> `outcome.py` → `runview.py` → `GET …/runview` (app.py) → `getRunView` (api.ts) → **CONTRACT GATE (byte-diff vs runview.ts + fixtures)** → FE load-effect rewrite (play-granular live ends here) → `store.create_attempt/add_turn/update_attempt` → `_run_one` per-turn + `max_turns` → `/scan`+`intensity` → **(one migration)** `Run.recon` → recon write + full ReconView. Waves 1–2 touch **neither** `runner.py` nor keystones (fully independent). **Exactly ONE migration** (`Run.recon`, Wave 4); everything else projects from existing rows.
>
> ### Vertical waves (each compiles + leaves the app working; 3–4 consolidated commits)
> - **WAVE 1 — BE-only RunView projection** *(no FE flip; app still on fixtures)*: `autosentinx/outcome.py` (1:1 `deriveOutcome` + `RISK_BAND` + golden test) · `autosentinx/runview.py` (reuse `_oracle/_detector_hits/_bypass`; **new** snake regulation + model-retaining vote mappers; `build_arc`; D8 two-pass `pairedIdx`; status/summary; `engine.maxTurns` int; `recon:null`; render error/blocked) · `GET /console/runs/{id}/runview` (catalog+library, auth-gated) · `getRunView` in `api.ts` · **CONTRACT GATE**. No migration.
> - **WAVE 2 — FE live slice** *(additive source; fixtures preserved)*: rewrite the load effect in both pages to branch `data='live' → getRunView` vs fixture; keep `fromStateJson`+switcher+`RunNav`+dev-bridge; 2500ms poll + terminal-stop; handle 401. No migration.
> - **WAVE 3 — BE per-turn liveness + intensity dial** *(rebased on main runner.py, symbol-anchored)*: `store.create_attempt`(add+commit+refresh for the FK)/`add_turn`/`update_attempt`; `_run_one` creates a `running` attempt → `add_turn` per turn → finalize; `run_budget`/`_run_one` accept `max_turns` → `range(max_turns or self.s.max_turns)`; `/scan`+`ScanConfig`+`startScan` send `max_turns`+`intensity` (map `INTENSITY[level]`) → `Run.roe`. No migration (`outcome='running'` is free-text).
> - **WAVE 4 — Recon persistence + ReconView** *(optional, the ONE migration)*: `Run.recon` Alembic (`down_revision`=head, default `''`); runner writes `recon.model_dump_json()`; `runview.py` projects full `ReconView` with defensive `getattr(run,'recon','')`.
>
> **`[EP-R8]` runner.py forward-merge with keystones** — symbol-anchored edits; mechanical conflict only, no semantic change; the gateway-wrapped `target` is API-compatible.
>
> *(The EP-1…EP-16 body below is the detailed design; read it through the R1 lens above.)*
>
