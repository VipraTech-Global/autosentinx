...
async def main(args) -> None:
    ...
    if args.strategy in ("ucb", "random"):
        ...
        await Runner().run_budget(run.id, args.objectives, args.budget, args.strategy,
                                  modes=args.modes, csrt=args.csrt in ("on", "both"))
    else:  # exhaustive
        ...
        if not runspecs:
            ...
            return
        run = Run(target_url=s.aarav_base_url, note=f"exhaustive cli campaign ({len(runspecs)} runs)")
        await store.create_run(run)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--n", type=int, default=None, help="cap techniques per objective (exhaustive)")
    ap.add_argument("--csrt", default="off", choices=["off", "on", "both"])
    ap.add_argument("--include-draft", action="store_true")
    ap.add_argument("--limit", type=int, default=None, help="cap total runs (exhaustive)")
